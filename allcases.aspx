<%@ Page Language="C#" AutoEventWireup="true" CodeFile="allcases.aspx.cs" Inherits="allcases" %>
...
<head>
 ...
 <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.5.14/jspdf.plugin.autotable.min.js"></script>
	...
	<link rel="stylesheet" href="https://sambalpurforestdivision.iicaapp.co.in/css/styles.css">
    <link rel="stylesheet" href="https://sambalpurforestdivision.iicaapp.co.in/css/style.css">
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $.ajax({
                type: "POST",
                url: "allcases.aspx/Getdesp3",
                data: '{name: "' + $("#hdnbookx").val() + '" }',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessddlxry,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessddlxry(response) {
                //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                //alert("Created Service Successfully");

                $('#myDropdown1').html("");
                $('#myDropdown1').append('<option value="ND">Select</option>');
            ...
            $.ajax({
                type: "POST",
                url: "allcases.aspx/Getdesp4",
                data: '{name: "' + $("#hdnbookx").val() + '" }',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessddlxz,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessddlxz(response) {
                //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                //alert("Created Service Successfully");

                $('#myDropdown2').html("");
                $('#myDropdown2').append('<option value="ND">Select </option>');
            ...
            $.ajax({
                type: "POST",
                url: "allcases.aspx/Getdesp1",
                data: '{name: "' + $("#hdnbookx").val() + '" }',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessddlx33,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessddlx33(response) {
                //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                //alert("Created Service Successfully");
                ...
                $.each(response, function (index, item) {
                    for (var i in item) {
                        $('#tcss').append(' <tr><td>' + f + '</td><td>' + item[i].split('*')[0] + '</td><td>0&nbsp;<button type="button" class="btn btn-info">Manage</button></td><td>0&nbsp;<button type="button" class="btn btn-info">Manage</button></td><td>' + item[i].split('*')[1] + '</td><td>' + item[i].split('*')[2] + '</td><td>' + item[i].split('*')[3] + '</td><td>' + item[i].split('*')[4] + '</td><td><button type="button" onclick="fnpdfx3(\''+item[i].split('*')[0]+'\')" class="btn btn-primary">Download PDF</button><button type="button" class="btn btn-success">Delete</button><button type="button" class="btn btn-success">Resume</button></td> </tr>');
                        f++;
                    }
		...
		function fnpdfx3(ex) {
		//	var caseid= ex.parent().parent().find('td:eq(1)').html();
			console.log(ex);
			       pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.10.377/pdf.worker.min.js';

   async function getPdfAsImages(url) {
    let pdf;
 ...
 if (pdf && pdf.numPages > 0) {
    const images = [];
    const scaleFactor = 2; // Increase the scale for higher resolution
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
        ...
        // Use the scale factor to get a higher resolution viewport
        ...
        // Check if the canvas is not blank before pushing the image
        if (!isCanvasBlank(canvas)) {
            images.push(canvas.toDataURL('image/png')); // Add high-resolution non-blank images
        }
    }

    return images;
} else {
    console.error("Invalid or empty PDF.");
    return []; // Return an empty array if the PDF is null or has no pages
}
}
 function isCanvasBlank(canvas) {
        ...
        // Check if all pixels are white (255,255,255) or transparent (alpha=0)
        for (let i = 0; i < pixelData.length; i += 4) {
            ...
            // If a pixel is not white or fully transparent, canvas is not blank
...
async function mergePdfs(urls) {
    ...
    const imgDataArray = await getPdfAsImages(urls[i]);
    ...
    doc.save('CASE_SUMMARY_WILDLIFE_'+ex+'.pdf');
}

   const urls = [
      "rqtnwtns"+ex+".pdf",
    "deputationoder"+ex+".pdf",
    "cmndcertfcte"+ex+".pdf",
    "searchwrnt"+ex+".pdf",
    "beforesearch"+ex+".pdf",
    "forms3"+ex+".pdf",
    ...
    "fircopy"+ex+".pdf",
    "forms5"+ex+".pdf",
    "ckwildlifeofnc"+ex+".pdf",
    "reportingdfo"+ex+".pdf",
    "appointmntinvofcr"+ex+".pdf",
    "carcassmemo"+ex+".pdf",
    "dforprt"+ex+".pdf",
    "rqstnfrpmortem"+ex+".pdf",
    "dispsalcarcs"+ex+".pdf",
    "statemntwitns"+ex+".pdf",
    "statemntacsed"+ex+".pdf",
    "crimerecrtnmemo"+ex+".pdf",
    "szrememospot"+ex+".pdf",
    "prlimnarypstmrtmrepo"+ex+".pdf",
    "rqsnchemex"+ex+".pdf",
    "exhibitfrdingcncrninsn"+ex+".pdf",
    "exhibtnfrnsicexmn"+ex+".pdf",
    "rqtnmdexm"+ex+".pdf",
    "rwtnmdclexmacsd"+ex+".pdf",
    "acusedfrwding"+ex+".pdf",
    "finalpmstemrepo"+ex+".pdf",
    "crimedtlspotmp"+ex+".pdf",
    "chemicalexrepo"+ex+".pdf",
    "frnscexmnrepo"+ex+".pdf",
    "rqstntosp"+ex+".pdf",
    "sec65b"+ex+".pdf",
    "cdrepotrqsn"+ex+".pdf",
    "cdrepo65b"+ex+".pdf",
    ...
    "casediary"+ex+".pdf",
    "cwhrepot"+ex+".pdf",
    "electricalinsreo"+ex+".pdf",
    "revenueinsprepo"+ex+".pdf",
    "characteranteced"+ex+".pdf",
    "corrobortngevd"+ex+".pdf",
    "prosectnrepo"+ex+".pdf"
       ...
       mergePdfs(urls);
        ...
        function fnpdfx() {
            ...
            doc.text('STATE FOREST DIVISON', pageWidth / 2, margin + 55, { align: 'center' }); // Center and adjust y position

            doc.setFontSize(12); // Slightly larger font size for the address
            doc.text('FOREST/WILDLIFE', pageWidth / 2, margin + 65, { align: 'center' }); // Center and adjust y position
            doc.text('SAMBALPUR, ODISHA, 768001', pageWidth / 2, margin + 80, { align: 'center' });
            doc.text('Phone: (123) 456-7890', pageWidth / 2, margin + 95, { align: 'center' });

            // Add a heading
            doc.setFontSize(24);
            doc.setTextColor(0, 51, 102); // Dark blue color
            doc.text('CASE SYNOPSIS', pageWidth / 2, 130, { align: 'center' }); // Adjusted y position for heading
            ...
            localStorageKeys.forEach(key => {
                ...
                if (jsonData && Array.isArray(jsonData) && jsonData.length > 0) {
                    ...
                    if (headerIndex < headers.length) {
                        ...
                        yPosition += headerHeight +20; // Add space after the header (adjust if needed)
                    ...
                    jsonData.forEach(item => {
                        ...
                        if (item.pic) {
                            ...
                            const imgWidth = doc.internal.pageSize.getWidth()-20; // Set desired width
                            const imgHeight = doc.internal.pageSize.getHeight()-20; // Set a default height or calculate based on the aspect 
 							 const imgX = (pdfWidth - imgWidth) / 2;
							     doc.rect(imgX - 10, yPosition - 10, imgWidth + 10, imgHeight + 10);
                            doc.addImage(item.pic, 'JPEG', imgX, yPosition, imgWidth, imgHeight);
      ...
      <th>Officer Name</th>
       <th>Options</th>
        ...
        <tbody id="tcss">
          ...
          <script src="https://sambalpurforestdivision.iicaapp.co.in/js/script.js"></script>
