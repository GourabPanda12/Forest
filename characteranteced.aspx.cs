...
public partial class characteranteced : System.Web.UI.Page
{
    ...
    public static string Insert(string cmndctno, string firno, string pic, string path)
    {
        ...
        if (path != null && path != string.Empty)
        {
            ...
            {
                //Create FTP Request.
                FtpWebRequest request = (FtpWebRequest)WebRequest.Create(ftpFolder + path.ToString().Split('.')[0] + "tbl_fircopy" + "." + path.ToString().Split('.')[1].ToString());
        ...
        {
            using (SqlCommand cmd = new SqlCommand("INSERT INTO [tbl_fircopy] (cmdno,firno,upldfile) VALUES (@cmdno,@firno,@upldfile)"))
            {
                cmd.Parameters.AddWithValue("@cmdno", cmndctno);
                cmd.Parameters.AddWithValue("@firno", firno);
                cmd.Parameters.AddWithValue("@upldfile", "https://iicaapp.co.in/httpdocs/cmndcrt" + path.ToString().Split('.')[0] + "fircpy" + "." + path.ToString().Split('.')[1].ToString());
                cmd.Connection = con;
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
    ...
    public static List<chrantData> GetAutoCompleteData(string prefix)
    {
        List<chrantData> services = new List<chrantData>();
