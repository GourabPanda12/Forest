<%@ Page Language="C#" AutoEventWireup="true" CodeFile="dossiervchl2.aspx.cs" Inherits="dossiervchl2" %>
...
@media (max-width: 768px) {
    ...
    <script>
        $(document).ready(function () {
            $.ajax({
                url: "dossiervchl2.aspx/GetSeizureDetails", // Adjust the endpoint as needed
                method: "POST",
                ...
                success: function (response) {
                    ...
                    data.forEach(item => {
                        tableRows += `<tr>
                            ...
                            <td>${item.OwnerFatherName}</td>
                            <td>${item.OwnerAddress}</td>
                            <td>${item.OwnerContact}</td>
                            <td>${item.OwnerEmail}</td>
                            <td>${item.OwnerId}</td>
                            <td>${new Date(item.DateSeizure).toLocaleDateString()}</td>
                            <td>${item.LocationSeizure}</td>
                            <td>${item.ReasonSeizure}</td>
                            <td>${item.AuthoritySeized}</td>
                            <td>${item.CurrentLocation}</td>
                            <td>${item.VehicleType}</td>
                            <td>${item.EngineNumber}</td>
                            <td>${item.ChassisNumber}</td>
                            <td>${item.ConditionVehicle}</td>
                            <td>${item.AdditionalFeatures}</td>
                            <td>${item.RC}</td>
                            <td>${item.Insurance}</td>
                            <td>${item.PUC}</td>
                            <td>${item.OtherDocuments}</td>
                            <td>${item.PreviousOffenses}</td>
                            <td>${item.CaseNumber}</td>
                            <td>${item.Court}</td>
                            <td>${item.InvestigatingOfficer}</td>
                            <td>${item.BriefCase}</td>
                            <td>${item.VehicleUsage}</td>
                            <td>${item.RegionsOperated}</td>
                            <td>${item.AcquittalsConvictions}</td>
...
<td>${item.ImagePaths ? `<a href="${item.ImagePaths}" target="_blank"><img src="${item.ImagePaths}" alt="Offender Image" width="50" height="50" /></a>` : 'No Image'}</td>

                <td> <button type="button" class="btn btn-info btn-sm" onclick="generateVehicleSeizurePDF(this)">Print</button></td>

                        </tr>`;
                    });
                    $("#seizureTable tbody").html(tableRows);
                },
        ...
        function generateVehicleSeizurePDF(button) {
            const row = $(button).closest("tr"); // Get the closest table row for the clicked button
            const cells = row.find("td");       // Get all the cells in that row

            const { jsPDF } = window.jspdf; // Ensure jsPDF is included
            const doc = new jsPDF();

            // Set title for the document
            doc.setFont("times", "bold");
            doc.setFontSize(14);
            doc.text("Vehicle Seizure Report", 105, 10, { align: "center" });

            // Collect data for the specific row (excluding SerialNo)
            const rowData = {
                MakeModel: cells.eq(1).text(),
                VIN: cells.eq(2).text(),
                RegistrationNumber: cells.eq(3).text(),
                Color: cells.eq(4).text(),
                YearManufacture: cells.eq(5).text(),
                OdometerReading: cells.eq(6).text(),
                OwnerName: cells.eq(7).text(),
                OwnerFatherName: cells.eq(8).text(),
                OwnerAddress: cells.eq(9).text(),
                OwnerContact: cells.eq(10).text(),
                OwnerEmail: cells.eq(11).text(),
                OwnerId: cells.eq(12).text(),
                DateSeizure: cells.eq(13).text(),
                LocationSeizure: cells.eq(14).text(),
                ReasonSeizure: cells.eq(15).text(),
                AuthoritySeized: cells.eq(16).text(),
                CurrentLocation: cells.eq(17).text(),
                VehicleType: cells.eq(18).text(),
                EngineNumber: cells.eq(19).text(),
                ChassisNumber: cells.eq(20).text(),
                ConditionVehicle: cells.eq(21).text(),
                AdditionalFeatures: cells.eq(22).text(),
                RC: cells.eq(23).text(),
                Insurance: cells.eq(24).text(),
                PUC: cells.eq(25).text(),
                OtherDocuments: cells.eq(26).text(),
                PreviousOffenses: cells.eq(27).text(),
                CaseNumber: cells.eq(28).text(),
                Court: cells.eq(29).text(),
                InvestigatingOfficer: cells.eq(30).text(),
                BriefCase: cells.eq(31).text(),
                VehicleUsage: cells.eq(32).text(),
                RegionsOperated: cells.eq(33).text(),
                AcquittalsConvictions: cells.eq(34).text(),
                AdditionalNotes: cells.eq(35).text(),
            };

            const signatureImage = cells.eq(36).find("img").attr("src");
            const vehicleImage = cells.eq(37).find("img").attr("src");

            doc.setFont("times", "normal");
            doc.text("Vehicle Photo", 90, 30); // Position the text within the rectangle
            doc.rect(70, 20, 70, 40); // Draw a rectangle (x, y, width, height)
            doc.addImage(vehicleImage, 'JPEG', 86, 21, 37, 37);
            ...
            doc.autoTable({
                ...
                startY: 75,                 // Start Y position
                styles: {
                    font: "times",
                    fontSize: 12,            // Font size for the table content
                    textColor: 0,            // Black text
                    cellPadding: 3,          // Add padding for cells
                ...
                margin: { top: 10, left: 10, right: 10 }, // Adjust margins if necessary
            });
            ...
            if (signatureImage) {
                ...
                doc.addImage(signatureImage, "JPEG", 10, finalY + 5, 50, 15);
                finalY += 60;
            } else {
            ...
            // Save the PDF
            doc.save(`Vehicle_Seizure_Report_Row_${cells.eq(0).text()}.pdf`);
        }
   ...
   <section id="content">
       ...
       <main>
          <div class="container">
            ...
            <h2 style="text-align:center; font-weight:500; margin:30px 0; color:#fff">Vehicle Report</h3>
        <table id="seizureTable" class="table table-striped table-bordered display nowrap" style="width:100%">
            <thead>
                <tr>
                    <th>Serial No</th>
                    <th>Make Model</th>
                    ...
                    <th>Owner Name</th>
                    <th>Owner Father's Name</th>
                    <th>Owner Address</th>
                    <th>Owner Contact</th>
                    <th>Owner Email</th>
                    <th>Owner ID</th>
                    <th>Date Seizure</th>
                    <th>Location Seizure</th>
                    <th>Reason Seizure</th>
                    <th>Authority Seized</th>
                    <th>Current Location</th>
                    <th>Vehicle Type</th>
                    <th>Engine Number</th>
                    <th>Chassis Number</th>
                    <th>Condition Vehicle</th>
                    ...
                    <th>Brief Case</th>
                    <th>Vehicle Usage</th>
                    <th>Regions Operated</th>
                    <th>Acquittals Convictions</th>
