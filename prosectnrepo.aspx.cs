...
public partial class prosectnrepo : System.Web.UI.Page
{
    ...
    public static string Insert(string caseno, string pic, string path)
    {
        ...
        if (path != null && path != string.Empty)
        {
            ...
            {
                //Create FTP Request.
                FtpWebRequest request = (FtpWebRequest)WebRequest.Create(ftpFolder + path.ToString().Split('.')[0] + "finalrepo" + "." + path.ToString().Split('.')[1].ToString());
        ...
        {
            using (SqlCommand cmd = new SqlCommand("INSERT INTO [tbl_prosectnrpt] (caseno,upldfile) VALUES (@caseno,@upldfile)"))
            {
                cmd.Parameters.AddWithValue("@caseno", caseno);
                cmd.Parameters.AddWithValue("@upldfile", "https://iicaapp.co.in/httpdocs/cmndcrt" + path.ToString().Split('.')[0] + "finalrepo" + "." + path.ToString().Split('.')[1].ToString());
                cmd.Connection = con;
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
