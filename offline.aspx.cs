...
public partial class offline : System.Web.UI.Page
{
    ...
    public static string Insert(string descrip, string pic, string path)
    {
        ...
        if (path != null && path != string.Empty)
        {
            ...
            {
                //Create FTP Request.
                FtpWebRequest request = (FtpWebRequest)WebRequest.Create(ftpFolder + path.ToString().Split('.')[0] + "scheduleform" + "." + path.ToString().Split('.')[1].ToString());
        ...
        {
            using (SqlCommand cmd = new SqlCommand("INSERT INTO [tbl_schedlfrm] (descp,upldfile) VALUES (@descp,@upldfile)"))
            {
                cmd.Parameters.AddWithValue("@descp", descrip);
                cmd.Parameters.AddWithValue("@upldfile", "https://iicaapp.co.in/httpdocs/cmndcrt" + path.ToString().Split('.')[0] + "scheduleform" + "." + path.ToString().Split('.')[1].ToString());
                cmd.Connection = con;
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
