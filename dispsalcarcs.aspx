﻿<%@ Page Language="C#" AutoEventWireup="true" CodeFile="dispsalcarcs.aspx.cs" Inherits="dispsalcarcs" %>
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"
 ...
 <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.5.14/jspdf.plugin.autotable.min.js"></script>
	...
	<title>Disposal Of Carcass</title>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function handleFileChange(event) {
                const input = event.target; // The specific input element triggering the event
                const key = $(input).attr('id'); // Dynamically get the unique ID of the input element

                if (input.files.length > 0) {
                    const file = input.files[0]; // Get the selected file

                    readFileAsBase64(file).then((base64String) => {
                        // Check if the key already exists in the JSON array
                        const existingIndex = jsonArray.findIndex(item => item.key === key);
                        if (existingIndex !== -1) {
                            // Update the existing entry
                            jsonArray[existingIndex].value = base64String;
                        } else {
                            // Add a new entry
                            jsonArray.push({ key, value: base64String });
                        }
            ...
            $('#txto').on('change', handleFileChange);
            $('#txtdfocum').on('change', handleFileChange);
            $('#third').on('change', handleFileChange);
            $('#fourth').on('change', handleFileChange);
            ...
            $('#txtcaseno').autocomplete({
                source: function (request, response) {
                    $.ajax({
                        type: "POST",
                        url: "dispsalcarcs.aspx/GetAutoCompleteData",
       ...
       // const jsonArray = [];
        ...
        // Attach the event listener to all file inputs dynamically using jQuery
        ...
        function sub() {
            ...
            var letterno = $("#txtltrno").val();
            var date = $("#txtdt").val();
            var refcaseno = $("#txtref").val();
            var caseof = $("#txtof").val();
            var officeodno = $("#txtodrno").val();
            var dated = $("#txtdtd").val();
            var warden = $("#txtwrdn").val();
            ...
            $.ajax({
                type: "POST",
                url: "dispsalcarcs.aspx/Insert",
                data: JSON.stringify({ discarcasslist: discarcasslist, pic: $("#hiden").val(), path: $("#hidden").val() }),
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (response) {
                    $("#alertdvd1").show();
        ...
        function print() {
            ...
            doc.text("Disposal Of Carcass", pdfWidth / 2, topMargin + 25, { align: "center" });
            ...
            const letterNo = $('#txtltrno').val();
            const date = $('#txtdt').val();
            const refNo = $('#txtref').val();
            const of = $('#txtof').val();
            const rangeOfficerFile = $('#txto').prop('files')[0];
            const dfoFile = $('#txtdfocum').prop('files')[0];

            // Define a function to convert image file to base64
            ...
            function addImageToPDF(doc, imgData, x, y, width, height) {
          ...
          //  var pageWidth = doc.internal.pageSize.getWidth(); // Get the page width
            doc.setFontSize(16);
            doc.text('Direction of authority for disposal of Carcass', pageWidth / 2, 50, { align: 'center' });
            doc.setFontSize(14);
            doc.text('Office of the Divisional Forest Officer-cum-Wildlife Warden', pageWidth / 2, 65, { align: 'center' });


            addTextToPDF(doc, `Letter No: ${letterNo}`, 50, 100);
            addTextToPDF(doc, `Date: ${date}`, 50, 115);
            addTextToPDF(doc, `Ref.OR Case No: ${refNo}`, 50, 130);
            addTextToPDF(doc, `Of: ${of}`, 50, 145);

            getBase64Image(rangeOfficerFile, function (imgData) {
                addImageToPDF(doc, imgData, 10, 260, 50, 50);
            });

            //   addTextToPDF(doc, 'You are hereby directed to send the carcass for post-mortem examination...', 10, 140);

            getBase64Image(dfoFile, function (imgData) {
                addImageToPDF(doc, imgData, 10, 310, 50, 50);
            ...
            var orderNo = $('#txtodrno').val() || "__________"; // Order number
            var daten = $('#txtdtd').val() || "__________";      // Date
            var warden = $('#txtwrdn').val() || "____________"; // Warden name
            ...
            doc.text("Range Officer", 280, 180); // Set x to 100 and y to 270


            doc.setFontSize(12);
            doc.setFont("helvetica", "normal");
            doc.text("    You are hereby directed to send the carcass for post-mortem \n" +
                         "examination of and after examination, the carcass of\n" +
                         "the animal be buried in the presence of witnesses and \n"+
                         "the certificate of the disposal be kept in record.", 180, 200);
            // Add Title
            doc.setFontSize(14); // Set font size
            doc.setFont("helvetica", "bold");
            doc.text("D.F.O-cum Wildlife Warden", 265, 260);
          
          //  doc.line(0, 650, 120, 270, 'D');
            doc.line(0, 270, 450, 270);
          //  doc.addPage();
            doc.setFontSize(16); // Set font size
            doc.setFont("helvetica", "bold"); // Set font to bold
            doc.text("Certificate of Disposal of Carcass", pageWidth / 2, 300, { align: 'center' });
            ...
            // Add the main content (orderNo, date, warden from input fields)
            doc.setFontSize(12);
            doc.setFont("helvetica", "normal");
            doc.text("Certified that as per the direction vide office order No-" + orderNo, 40, 330);

            // Dynamic input values inserted here
           // doc.text(, 290, 60);  // Order No from input field
            doc.text("dated       " + daten + "  of Wild life Warden   " + warden , 40, 340);
         //   doc.text(, 100, 75);     // Date from input field
           // doc.text(, 40, 90);
          //  doc.text(, 140, 90);   // Warden name from input field
            doc.text("Division the carcass of animal-(name) after its post mortem", 40, 360);
            doc.text(" examination was disposed off in presence of following witnesses.", 40, 370);

            doc.setFontSize(14); // Set font size
            doc.setFont("helvetica", "bold");
            doc.text("Range Officer", 280, 420);

            doc.setFontSize(14); // Set font size
            doc.setFont("helvetica", "bold");
            doc.text("D.F.O-cum Wildlife Warden", 265, 460);




            for (let i = 0; i < jsonArray.length; i++) {
                const imgData = `data:image/jpeg;base64,${jsonArray[i].value}`;
                const xPos = 290; // Bottom-right corner X position
                var yPos = i % 2 === 0 ? 150 : 235; // Adjust Y position for image placement
                const width = 50; // Image width
                const height = 15; // Image height

                if (i == 2) {
                    yPos += 285;
                   // doc.addPage(); // Add new page after placing two images
                }
                if (i == 3) {
                    yPos += 160;
                    // doc.addPage(); // Add new page after placing two images
                }
                doc.addImage(imgData, 'JPEG', xPos, yPos, width, height);
            }
            ...
            rqtn.forEach(item => {
                ...
                const imgHeight = doc.internal.pageSize.getHeight(); // Image height
            ...
            doc.save('Disposla of dispsalcarcs.pdf');
...
<h2 class="text-center" style="color: #fff;">Disposal Of Carcass</h2>
             ...
             <div class="row " style="margin-top:50px;">
              ...
              <h6 class="text-center mb-3" style="color:#000;">Direction of authority for disposal of Carcass. </h6>
<h5 style="color:#000;" class="text-center">Office of the Divisional Forest Officer-cum-Wildlife Warden</h5>



<div class="col-lg-6 col-sm-6 col-xs-6 abc">
  <label class="lebl">Letter No...</label>
   <input type="text" id="txtltrno" class="form-control console savedt">

       </div>
                                           
 <div class="col-lg-6 col-sm-6 col-xs-6 abc">
  <label class="lebl">Date..</label>
   <input type="date" id="txtdt" class="form-control console savedt">
       </div>

                 
<div class="col-lg-6 col-sm-6 col-xs-6 abc">
 
    <label class="lebl">Ref.OR Case No...</label>
   <input type="text" id="txtref" class="form-control console savedt">

       </div>


<div class="col-lg-6 col-sm-6 col-xs-6">
      <label class="lebl">Of</label>
     <input type="text" id="txtof" class="form-control console savedt">
    </div>
                    
                             
<div class="col-lg-6 col-sm-6 col-xs-6 mt-3">
  <input type="file" id="txto" class="form-control console savedt" >
...
<p style="color:#000;">You are hereby directed to send the carcass for post-mortem examination of and after examination, the carcass of the animal be buried in the presence of witnesses and the certificate of the disposal be kept in record.</p>

</div>
                                                <div class="col-lg-6 col-sm-6 col-xs-6 mt-3 mb-3">
 
 <input type="file" id="txtdfocum" class="form-control console savedt" >
    <label class="lebl">D.F.O-cum Wildlife Warden</label>
...
<h5 style="color:#000;" class="text-center">Certificate of Disposal of Carcass</h5>

<label>Certified that as per the direction vide office order No-<input type="text" id="txtodrno" class="savedt" />dated <input type="date" id="txtdtd" class="savedt" />of Wild life Warden<input type="text" id="txtwrdn" class="savedt" />
    Division the carcass of animal-(name) after its post mortem examination was disposed off in presence of following witnesses.
</label>
...
<div class="col-lg-6 col-sm-6 col-xs-6">
     ...
     <input type="file" id="third" class="form-control console savedt">
    </div>
          
 
                                          
                             
<div class="col-lg-6 col-sm-6 col-xs-6 mt-4 mb-3">
  <input type="file" id="fourth" class="form-control console savedt">

    <label class="lebl">Signature of witnesses with date</label>
...
"><i>(Supported image file formats - jpeg/png/jpg)</i></abbr></label>
  ...
  <div id="imagePreviewContainer" class="image-preview-container"></div>
                             ...
                             <div class="col-lg-3  col-sm-3 col-xs-3 abc">
        ...
        <button class="btnx mt-4" type="button" onclick="sub()" >Print</button>
