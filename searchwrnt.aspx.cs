...
public partial class searchwrnt : System.Web.UI.Page
{
    ...
    public static string Insert(string caseno,string pic, string path)
    {
        ...
        if (path != null && path != string.Empty)
        {
            ...
            {
                //Create FTP Request.
                FtpWebRequest request = (FtpWebRequest)WebRequest.Create(ftpFolder + path.ToString().Split('.')[0] + "searchwarant" + "." + path.ToString().Split('.')[1].ToString());
        ...
        {
            using (SqlCommand cmd = new SqlCommand("INSERT INTO [tbl_srchwrnt] (cmdno,upldfile) VALUES (@cmdno,@upldfile)"))
            {
                cmd.Parameters.AddWithValue("@cmdno", caseno);
              
                cmd.Parameters.AddWithValue("@upldfile", "https://iicaapp.co.in/cmndcrt/" + path.ToString().Split('.')[0] + "searchwarant" + "." + path.ToString().Split('.')[1].ToString());
                cmd.Connection = con;
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
    ...
    public static List<ReqDatx> GetAutoCompleteData(string prefix)
    {
        List<ReqDatx> services = new List<ReqDatx>();
