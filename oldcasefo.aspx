﻿<%@ Page Language="C#" AutoEventWireup="true" CodeFile="oldcasefo.aspx.cs" Inherits="oldcasefo" %>


<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Old Case</title>
    <link href='https://unpkg.com/boxicons@2.0.9/css/boxicons.min.css' rel='stylesheet'>

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"
        integrity="sha512-SfTiTlX6kk+qitfevl/7LibUOeJWlt9rbyDn92a1DqWOw9vWG2MFoays0sgObmWazO5BQPiFucnnEAjpAB+/Sw=="
        crossorigin="anonymous" referrerpolicy="no-referrer" />


    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">

    <link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="css/style.css">

         <link rel="stylesheet" href="https://code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css"/>
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
    <link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    
    <script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.3/dist/umd/popper.min.js"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.3.1/jspdf.umd.min.js"></script>
 <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.5.14/jspdf.plugin.autotable.min.js"></script>
    
	<link rel="stylesheet" href="css/styles.css">
    <link rel="stylesheet" href="css/style.css">
    <script type="text/javascript">
        const hdr = localStorage.getItem('hdrsec');
        var jsonDynmxx = JSON.parse(localStorage.getItem('rqtnx1'));
        $(document).ready(function () {
            var hrefArray = [];

            // Iterate over each 'a' element within #divpnlx
            $('#divpnlx a').each(function () {
                var hrefValue = $(this).attr('href');

                // Split the href value at the dot and take the first part
                var firstPart = hrefValue.split('.')[0];

                // Add the first part to the array
                hrefArray.push(firstPart);
            });

            // Log the result
            console.log(hrefArray);
            const pdfFiles = hrefArray.map(file => `${file}.pdf`);
            console.log(pdfFiles);

            var j = 1;
            const headers = [
                { header: "Requisition For Independent Witness", content: "Content for Requisition For Independent Witness..." },
                { header: "Deputation Order Of Witness By Concerned Authority", content: "Content for Deputation Order Of Witness..." },
                { header: "Command Certificate", content: "Content for Command Certificate..." },
                { header: "Search warrant by ACF", content: "Content for Search warrant by ACF..." },
                { header: "Before Search Protocol", content: "Content for Before Search Protocol..." },
                { header: "Seizure List", content: "Content for Seizure List..." },
                { header: "After Search Protocol", content: "Content for After Search Protocol..." },
                { header: "Property Register Number MR No/CMR No", content: "Content for Property Register Number MR No/CMR No..." },
                { header: "FIR copy", content: "Content for FIR copy..." },
                { header: "Arrest Memo", content: "Content for Arrest Memo..." },
                { header: "Checklist For Wildlife Offence", content: "Content for Checklist For Wildlife Offence..." },
                { header: "Reporting to DFO cum wildlife warden by Range officer regarding registration of case", content: "Content for Reporting to DFO cum wildlife warden..." },
                { header: "Appointment of Investigating Officer", content: "Content for Appointment of Investigating Officer..." },
                { header: "Carcass Inspection Memo", content: "Content for Carcass Inspection Memo..." },
                { header: "Dfo/Jtf/Other Experts", content: "Content for Dfo/Jtf/Other Experts..." },
                { header: "Requisition for post mortem examination to Veterinary officer", content: "Content for Requisition for post mortem examination..." },
                { header: "Disposal Of Carcass", content: "Content for Disposal Of Carcass..." },
                { header: "Statement Of Witness", content: "Content for Statement Of Witness..." },
                { header: "Statement Of Accused", content: "Content for Statement Of Accused..." },
                { header: "Crime Scene recreation memo by accused and statements of independent witnesses", content: "Content for Crime Scene recreation memo..." },
                { header: "Seizure memo of spot/weapon/trap led to discovery by accused", content: "Content for Seizure memo of spot/weapon/trap..." },
                { header: "Preliminary post mortem report by the concerned veterinary authority", content: "Content for Preliminary post mortem report..." },
                { header: "Requisition for chemical examination in concerned scientific lab", content: "Content for Requisition for chemical examination..." },
                { header: "Exhibit forwarding letter by concerned magistrate to Concerned institution such as SFSL/ZSI/WII for chemical/biological/ballistic examination u/s 293 CrPC", content: "Content for Exhibit forwarding letter..." },
                { header: "Exhibit forwarding letter by concerned magistrate for Forensic Examination for Identification of Species/sex etc by concerned institution such as ZSI/WII.", content: "Content for Exhibit forwarding letter for Forensic Examination..." },
                { header: "Requisition for medical examination of the accused", content: "Content for Requisition for medical examination..." },
                { header: "Inspection Memo of the accused by Medical officer", content: "Content for Inspection Memo of the accused..." },
                { header: "Accused Forwarding", content: "Content for Accused Forwarding..." },
                { header: "Final Postmortem Report", content: "Content for Final Postmortem Report..." },
                { header: "Crime Details Form along with Spot map (along with RI inspection report/joint verification report, Notification of sanctuary/ national park & notification of RF BLOCK included in the sanctuary/national park notification)", content: "Content for Crime Details Form..." },
                { header: "Chemical Examination Report", content: "Content for Chemical Examination Report..." },
                { header: "Forensic Examination Report", content: "Content for Forensic Examination Report..." },
                { header: "Requisition to SP for Forensic examination of digital evidence such as photography and videography during seizure/crime scene recreation/ discovery of spot/weapon/trap by accused", content: "Content for Requisition to SP for Forensic examination..." },
                { header: "Section 65B Certificate Compliance by DFSL(photographs/videos)", content: "Content for Section 65B Certificate Compliance..." },
                { header: "CDR report requisition letter to RCCF concerned", content: "Content for CDR report requisition letter..." },
                { header: "CDR report along with 65 B certificate by concerned Nodal officer through Superintendent of Police concerned", content: "Content for CDR report along with 65 B certificate..." },
                { header: "Bail Order Of The Accused If Any", content: "Content for Bail Order Of The Accused..." },
                { header: "Case Diary", content: "Content for Case Diary..." },
                { header: "CWH Report", content: "Content for CWH Report..." },
                { header: "Electrical Inspector report", content: "Content for Electrical Inspector report..." },
                { header: "Revenue Inspector report", content: "Content for Revenue Inspector report..." },
                { header: "Character Antecedents", content: "Content for Character Antecedents..." },
                { header: "Corroborating Evidence-Summary", content: "Content for Corroborating Evidence-Summary..." },
                { header: "Prosecution Report/Complaint(Final)", content: "Content for Prosecution Report/Complaint(Final)..." }
            ];
            $.each(headers, function (index, item) {

                item.jsx = 'rqtnx' + j;
                j++;// Set the jsx property starting from 1 to the length of the array
            });
            console.log(headers);
            headers.forEach(function (item) {
                $('#headersTable tbody').append(
                    `<tr>
                <td>${item.header}</td>
                <td><a href="#" onclick="fnpdf('${item.header}','${item.jsx}')">View & Download PDF</a></td>
            </tr>`
                );
            });

            // Show the modal when button is clicked
            $('#openModalBtn').click(function () {
                $('#tableModal').show();
            });
            $('#btncls').click(function () {
                $('#tableModal').hide();
            });




            $('#btnunv').click(function () {
                const n = 44; // Replace with your desired value
                const headers = [
                    { header: "Requisition For Independent Witness", content: "Content for Requisition For Independent Witness..." },
                    { header: "Deputation Order Of Witness By Concerned Authority", content: "Content for Deputation Order Of Witness..." },
                    { header: "Command Certificate", content: "Content for Command Certificate..." },
                    { header: "Search warrant by ACF", content: "Content for Search warrant by ACF..." },
                    { header: "Before Search Protocol", content: "Content for Before Search Protocol..." },
                    { header: "Seizure List", content: "Content for Seizure List..." },
                    { header: "After Search Protocol", content: "Content for After Search Protocol..." },
                    { header: "Property Register Number MR No/CMR No", content: "Content for Property Register Number MR No/CMR No..." },
                    { header: "FIR copy", content: "Content for FIR copy..." },
                    { header: "Arrest Memo", content: "Content for Arrest Memo..." },
                    { header: "Checklist For Wildlife Offence", content: "Content for Checklist For Wildlife Offence..." },
                    { header: "Reporting to DFO cum wildlife warden by Range officer regarding registration of case", content: "Content for Reporting to DFO cum wildlife warden..." },
                    { header: "Appointment of Investigating Officer", content: "Content for Appointment of Investigating Officer..." },
                    { header: "Carcass Inspection Memo", content: "Content for Carcass Inspection Memo..." },
                    { header: "Dfo/Jtf/Other Experts", content: "Content for Dfo/Jtf/Other Experts..." },
                    { header: "Requisition for post mortem examination to Veterinary officer", content: "Content for Requisition for post mortem examination..." },
                    { header: "Disposal Of Carcass", content: "Content for Disposal Of Carcass..." },
                    { header: "Statement Of Witness", content: "Content for Statement Of Witness..." },
                    { header: "Statement Of Accused", content: "Content for Statement Of Accused..." },
                    { header: "Crime Scene recreation memo by accused and statements of independent witnesses", content: "Content for Crime Scene recreation memo..." },
                    { header: "Seizure memo of spot/weapon/trap led to discovery by accused", content: "Content for Seizure memo of spot/weapon/trap..." },
                    { header: "Preliminary post mortem report by the concerned veterinary authority", content: "Content for Preliminary post mortem report..." },
                    { header: "Requisition for chemical examination in concerned scientific lab", content: "Content for Requisition for chemical examination..." },
                    { header: "Exhibit forwarding letter by concerned magistrate to Concerned institution such as SFSL/ZSI/WII for chemical/biological/ballistic examination u/s 293 CrPC", content: "Content for Exhibit forwarding letter..." },
                    { header: "Exhibit forwarding letter by concerned magistrate for Forensic Examination for Identification of Species/sex etc by concerned institution such as ZSI/WII.", content: "Content for Exhibit forwarding letter for Forensic Examination..." },
                    { header: "Requisition for medical examination of the accused", content: "Content for Requisition for medical examination..." },
                    { header: "Inspection Memo of the accused by Medical officer", content: "Content for Inspection Memo of the accused..." },
                    { header: "Accused Forwarding", content: "Content for Accused Forwarding..." },
                    { header: "Final Postmortem Report", content: "Content for Final Postmortem Report..." },
                    { header: "Crime Details Form along with Spot map (along with RI inspection report/joint verification report, Notification of sanctuary/ national park & notification of RF BLOCK included in the sanctuary/national park notification)", content: "Content for Crime Details Form..." },
                    { header: "Chemical Examination Report", content: "Content for Chemical Examination Report..." },
                    { header: "Forensic Examination Report", content: "Content for Forensic Examination Report..." },
                    { header: "Requisition to SP for Forensic examination of digital evidence such as photography and videography during seizure/crime scene recreation/ discovery of spot/weapon/trap by accused", content: "Content for Requisition to SP for Forensic examination..." },
                    { header: "Section 65B Certificate Compliance by DFSL(photographs/videos)", content: "Content for Section 65B Certificate Compliance..." },
                    { header: "CDR report requisition letter to RCCF concerned", content: "Content for CDR report requisition letter..." },
                    { header: "CDR report along with 65 B certificate by concerned Nodal officer through Superintendent of Police concerned", content: "Content for CDR report along with 65 B certificate..." },
                    { header: "Bail Order Of The Accused If Any", content: "Content for Bail Order Of The Accused..." },
                    { header: "Case Diary", content: "Content for Case Diary..." },
                    { header: "CWH Report", content: "Content for CWH Report..." },
                    { header: "Electrical Inspector report", content: "Content for Electrical Inspector report..." },
                    { header: "Revenue Inspector report", content: "Content for Revenue Inspector report..." },
                    { header: "Character Antecedents", content: "Content for Character Antecedents..." },
                    { header: "Corroborating Evidence-Summary", content: "Content for Corroborating Evidence-Summary..." },
                    { header: "Prosecution Report/Complaint(Final)", content: "Content for Prosecution Report/Complaint(Final)..." }
                ];
                // Generate the array of keys dynamically
                const localStorageKeys = [];
                for (let i = 1; i <= n; i++) {
                    localStorageKeys.push(`rqtnx${i}`);
                }
                console.log(localStorageKeys);

                const { jsPDF } = window.jspdf;

                // Create new jsPDF instance with portrait orientation, A4 size, and units in px
                const doc = new jsPDF({
                    orientation: 'portrait',
                    unit: 'px',
                    format: 'a4'
                });

                // HEADER SECTION

                // Set margins
                const margin = 10; // Increased margin
                const pageWidth = doc.internal.pageSize.getWidth();
                const pageHeight = doc.internal.pageSize.getHeight();
                const maxContentWidth = pageWidth - 2 * margin; // Maximum content width

                // Add background color
                doc.setFillColor(230, 230, 250); // Light lavender color
                doc.rect(0, 0, pageWidth, pageHeight, 'F');

                // Add header section
                const imgData = 'data:image/png;base64,'; // Replace with your logo's base64-encoded data
                const logoWidth = 60;  // Adjust logo width
                const logoHeight = 40; // Adjust logo height
                const logoX = (pageWidth - logoWidth) / 2; // Center logo horizontally
                doc.addImage(imgData, 'PNG', logoX, margin, logoWidth, logoHeight); // Position and size of the logo

                // Add address text
                doc.setFontSize(16); // Increase font size for the company name
                doc.setTextColor(40, 40, 40);
                doc.text('', pageWidth / 2, margin + 55, { align: 'center' }); // Center and adjust y position

                doc.setFontSize(12); // Slightly larger font size for the address
                doc.text('', pageWidth / 2, margin + 65, { align: 'center' }); // Center and adjust y position
                doc.text('', pageWidth / 2, margin + 80, { align: 'center' });
                doc.text('Phone: (123) 456-7890', pageWidth / 2, margin + 95, { align: 'center' });

                // Add a heading
                doc.setFontSize(24);
                doc.setTextColor(0, 51, 102); // Dark blue color
                doc.text('', pageWidth / 2, 130, { align: 'center' }); // Adjusted y position for heading
                const caseNumber = 'Case No:' + localStorage.getItem('caseno');
                doc.setFontSize(12);
                doc.setTextColor(0, 0, 0); // Black color
                doc.text(caseNumber, margin, margin + 10); // Top left corner

                // Add date to top right corner
                const currentDate = new Date().toLocaleDateString(); // Format the date as needed
                doc.text(`Date: ${currentDate}`, pageWidth - margin, margin + 10, { align: 'right' });



                let yPosition = 160;



                const pdfWidth = doc.internal.pageSize.getWidth();
                const pdfHeight = doc.internal.pageSize.getHeight();






                function createGradient(doc, startX, startY, endX, endY, startColor, endColor, steps) {
                    for (let i = 0; i <= steps; i++) {
                        const color = interpolateColor(startColor, endColor, i / steps);
                        doc.setFillColor(color[0], color[1], color[2]);
                        const yPosition = startY + (endY - startY) * (i / steps);
                        doc.rect(startX, yPosition, endX - startX, (endY - startY) / steps, 'F');
                    }
                }

                // Helper function to interpolate between two colors
                function interpolateColor(color1, color2, factor) {
                    return [
                        Math.round(color1[0] + (color2[0] - color1[0]) * factor),
                        Math.round(color1[1] + (color2[1] - color1[1]) * factor),
                        Math.round(color1[2] + (color2[2] - color1[2]) * factor)
                    ];
                }







                let headerIndex = 0; // Initialize header index

                localStorageKeys.forEach(key => {
                    const jsonData = JSON.parse(localStorage.getItem(key));
                    console.log(jsonData);

                    // Check if data exists for the key
                    if (jsonData && Array.isArray(jsonData) && jsonData.length > 0) {
                        // Add the header for the current section if headers are still available
                        if (headerIndex < headers.length) {
                            const currentHeader = headers[headerIndex].header;
                            const headerHeight = 30; // Height of the header background

                            // Create a gradient background for the header
                            const startColor = [0, 50, 100]; // Deep green-blue start color
                            const endColor = [0, 150, 200]; // Lighter green-blue end color
                            const steps = 50; // Number of gradient steps for smoother transition
                            createGradient(doc, 20, yPosition - headerHeight, pdfWidth - 20, yPosition, startColor, endColor, steps);

                            // Add the header text
                            doc.setFontSize(12);
                            doc.setTextColor(255, 255, 255); // Set text color to white for contrast
                            const headerLines = doc.splitTextToSize(currentHeader, pdfWidth - 40);
                            headerLines.forEach((line, index) => {
                                doc.text(line, pdfWidth / 2, yPosition - headerHeight + 15 + (index * 12), { align: "center" });
                            });

                            yPosition += headerHeight + 10; // Add space after the header (adjust if needed)

                            // Increment the header index to move to the next header in the next section
                            headerIndex++;
                        }

                        jsonData.forEach(item => {
                            // Example to add content
                            doc.setFontSize(10);
                            doc.text(item.caseno, 20, yPosition); // Start from a margin of 20
                            yPosition += 20; // Add space between contents

                            // Check if we need to add a new page
                            if (yPosition + 50 > pdfHeight) { // Adding some buffer for new content
                                doc.addPage(); // Add new page
                                yPosition = 20; // Reset y position for new page
                            }

                            // Example to add an image if any
                            if (item.pic) {
                                console.log(item.pic);

                                const imgWidth = 150; // Set desired width
                                const imgHeight = 100; // Set a default height or calculate based on the aspect ratio

                                doc.addImage(item.pic, 'JPEG', 20, yPosition, imgWidth, imgHeight);
                                yPosition += imgHeight + 20; // Add space after the image

                                // Check if we need to add a new page
                                if (yPosition + 50 > pdfHeight) {
                                    doc.addPage(); // Add new page
                                    yPosition = 20; // Reset y position for new page
                                }
                            }
                        });
                    }
                });













                doc.save('Case-Document.pdf');

            });



            $.ajax({
                type: "POST",
                url: "oldcasefo.aspx/getdwn",
                data: '{name: "nnmm" }',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessh3,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessh3(response) {
                //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                //alert("Created Service Successfully");

                // mtch = parseFloat(response.d.split('*')[0]);
                //  $('#trv').html("&#8377; " + response.d.split(',')[3]); 
                // 
                $('#totspn').html(response.d.split('*')[0]);
                //  $('#trvpx').html(response.d.split('*')[2]);
                //   $('#trvw').html("&#8377; " + mtch);
                //if (response.d.split('*')[3] === "Pending") {
                //    //alert(response.d.split('*')[2]);
                //}
                //else {
                //    //alert("ok");
                //    $("#btnkyc").css({
                //        "background": "radial-gradient(100% 100% at 100% 0, #CDDC39 0, #009688 100%)"

                //    });
                //    $("#btnkyc").html("KYC Activated !");
                //}

                // $('#trv').html("&#8377; " + mtch);
                //alert( parseFloat(response.d.split(',')[3]) - parseFloat(response.d.split(',')[6]));
                // $('#trvw').html("&#8377; " + parseFloat((response.d.split(',')[3]) - parseFloat(response.d.split(',')[6])).toFixed(2).toString());


            }


            $('#searchInput').on('keyup', function () {
                var value = $(this).val().toLowerCase();
                $('#divpnlx .col-sm-6').filter(function () {
                    $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
                });
            });


            $('#myDropdown2').select2({
                placeholder: 'Select an option',
                allowClear: true
            });
            $.ajax({
                type: "POST",
                url: "oldcasefo.aspx/Getdesp2",
                data: '{}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessddlx,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessddlx(response) {
                //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                //alert("Created Service Successfully");

                $('#myDropdown1').html("");
                $('#myDropdown1').append('<option value="ND">Choose Dist</option>');

                $.each(response, function (index, item) {
                    for (var i in item) {
                        $('#myDropdown1').append('<option value="' + item[i].split('*')[0] + '">' + item[i].split('*')[1] + '</option>');

                    }

                });

                $('#myDropdown1').select2({
                    placeholder: 'Select an option',
                    allowClear: true
                });
            }


            $('#myDropdown1').on('change', function () {

                $.ajax({
                    type: "POST",
                    url: "oldcasefo.aspx/Getdesp3",
                    data: '{name: "' + $("#myDropdown1 option:selected").val() + '" }',
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: OnSuccessddlx1,
                    failure: function (response) {
                        alert(response.d);
                    }
                });

                function OnSuccessddlx1(response) {
                    //$(".addsr").closest('form').find("input[type=text], textarea").val(""); $("#myDropdown option:selected").text();
                    //alert("Created Service Successfully");

                    $('#myDropdown2').html("");
                    $('#myDropdown2').append('<option value="ND">Choose Divison</option>');

                    $.each(response, function (index, item) {
                        for (var i in item) {
                            $('#myDropdown2').append('<option value="' + item[i].split('*')[0] + '">' + item[i].split('*')[1] + '</option>');

                        }

                    });
                    $('#myDropdown2').select2({
                        placeholder: 'Select an option',
                        allowClear: true
                    });

                }
            });

            $.ajax({
                type: "POST",
                url: "oldcasefo.aspx/Getdesp",
                data: '{name: "' + $("#hdnbookx").val() + '" }',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessddlx7,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessddlx7(response) {
                //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                //alert("Created Service Successfully");

                $('#myDropdown').html("");
                $('#myDropdown').append('<option value="ND">Choose Case</option>');

                $.each(response, function (index, item) {
                    for (var i in item) {
                        $('#myDropdown').append('<option value="' + item[i].split('*')[0] + '">' + item[i].split('*')[1] + '</option>');

                    }

                });

                $('#myDropdown').select2({
                    placeholder: 'Select an option',
                    allowClear: true
                });
            }



            $('#autocomplete').autocomplete({
                source: function (request, response) {
                    $.ajax({
                        type: "POST",
                        url: "oldcasefo.aspx/GetAutoCompleteData",
                        data: JSON.stringify({ prefix: request.term }),
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (data) {
                            response($.map(data.d, function (item) {
                                return {
                                    label: item.ServiceName,
                                    value: item.ServiceName,
                                    id: item.Id,
                                    categoryId: item.CategoryId,
                                    subCategoryId: item.SubCategoryId
                                };
                            }));
                        },
                        error: function (error) {
                            console.log("Error:", error);
                        }
                    });
                },
                select: function (event, ui) {
                    // Set the selected item ID, category ID, and subcategory ID in hidden fields
                    //$('#selectedServiceId').val(ui.item.id);
                    //$('#selectedServiceCategoryId').val(ui.item.categoryId);
                    //$('#selectedServiceSubCategoryId').val(ui.item.subCategoryId);
                    // location.href = "catgry.aspx?cubd=" + ui.item.categoryId + "&cnnm=" + ui.item.label;

                },
                minLength: 2 // Set the minimum length of input to trigger autocomplete
            });



            $('#myDropdown').on('change', function () {
                localStorage.setItem('caseno', $("#myDropdown option:selected").text());

                if ($("#myDropdown option:selected").text() != "Choose Case") {
                    $(".loading").show();
                    $("#divpnlx").show();
                    $("#csdtl").show();
                    $("#csdtl").html('');

                    $.ajax({
                        type: "POST",
                        url: "oldcasefo.aspx/Getdespp3",
                        data: '{name: "' + $("#myDropdown option:selected").text() + '" }',
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: OnSuccessddlx3,
                        failure: function (response) {
                            alert(response.d);
                        }
                    });

                    function OnSuccessddlx3(response) {
                        //$(".addsr").closest('form').find("input[type=text], textarea").val("");
                        //alert("Created Service Successfully");

                        // $('#myDropdown').html("");
                        // $('#myDropdown').append('<option value="ND">Choose Case</option>');

                        $.each(response, function (index, item) {
                            for (var i in item) {
                                $('#csdtl').append('<span>Divison : ' + item[i].split('*')[0] + '</span><span>District : ' + item[i].split('*')[1] + '</span><span>City : ' + item[i].split('*')[2] + '</span><span>Range Officer : ' + item[i].split('*')[3] + '</span><span>Description : ' + item[i].split('*')[4] + '</span><span>Date Of Initiation : ' + item[i].split('*')[5] + '</span>');

                            }

                        });

                        $('#myDropdown').select2({
                            placeholder: 'Select an option',
                            allowClear: true
                        });
                        $(".loading").hide();
                    }

                }

                else {
                    $("#divpnlx").hide();
                    $("#csdtl").hide();
                    alert("Please Choose a valid entry");
                }

            });

        });
        function sub() {
            $(".loading").show();

            $.ajax({
                type: "POST",
                url: "fircopy.aspx/Insert",
                data: '{ cmndctno:"' + $("#txtcno").val() + '",firno:"' + $("#txtfir").val() + '",pic:"' + $("#hiden").val() + '" ,path:"' + $("#hidden").val() + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: OnSuccessrrd,
                failure: function (response) {
                    alert(response.d);
                }
            });

            function OnSuccessrrd(response) {
                $(".file-upload-image").attr("src", "");
                $("#txtcno").val('');
                $("#hiden").val('');
                $("#hidden").val('');
                $(".loading").hide();
                $("#alertdvd1").show();

                // alert("You have Submited Fir Copy with respective case number successfully!"); txtcno

                // window.location.reload(); 
            }
        }

        function fnpdf(egh, epf) {
            // alert(e);
            var jsonDynmxx = JSON.parse(localStorage.getItem(epf));
            const { jsPDF } = window.jspdf;

            // Create new jsPDF instance with portrait orientation, A4 size, and units in px
            const doc = new jsPDF({
                orientation: 'portrait',
                unit: 'px',
                format: 'a4'
            });

            // HEADER SECTION

            // Set margins
            const margin = 10; // Increased margin
            const pageWidth = doc.internal.pageSize.getWidth();
            const pageHeight = doc.internal.pageSize.getHeight();
            const maxContentWidth = pageWidth - 2 * margin; // Maximum content width

            // Add background color
            doc.setFillColor(230, 230, 250); // Light lavender color
            doc.rect(0, 0, pageWidth, pageHeight, 'F');

            // Add header section
            const imgData = 'data:image/png;base64,'; // Replace with your logo's base64-encoded data
            const logoWidth = 60;  // Adjust logo width
            const logoHeight = 40; // Adjust logo height
            const logoX = (pageWidth - logoWidth) / 2; // Center logo horizontally
            doc.addImage(imgData, 'PNG', logoX, margin, logoWidth, logoHeight); // Position and size of the logo

            // Add address text
            doc.setFontSize(16); // Increase font size for the company name
            doc.setTextColor(40, 40, 40);
            doc.text('', pageWidth / 2, margin + 55, { align: 'center' }); // Center and adjust y position

            doc.setFontSize(12); // Slightly larger font size for the address
            doc.text('', pageWidth / 2, margin + 65, { align: 'center' }); // Center and adjust y position
            doc.text('', pageWidth / 2, margin + 80, { align: 'center' });
            doc.text('Phone: (123) 456-7890', pageWidth / 2, margin + 95, { align: 'center' });

            // Add a heading
            doc.setFontSize(24);
            doc.setTextColor(0, 51, 102); // Dark blue color
            doc.text('', pageWidth / 2, 130, { align: 'center' }); // Adjusted y position for heading
            const caseNumber = 'Case No:' + localStorage.getItem('caseno');
            doc.setFontSize(12);
            doc.setTextColor(0, 0, 0); // Black color
            doc.text(caseNumber, margin, margin + 10); // Top left corner

            // Add date to top right corner
            const currentDate = new Date().toLocaleDateString(); // Format the date as needed
            doc.text(`Date: ${currentDate}`, pageWidth - margin, margin + 10, { align: 'right' });



            // END


            //  INTERNAL SECTION


            const pdfWidth = doc.internal.pageSize.getWidth();
            const topMargin = 10;
            const gradientHeight = 30; // Height of the gradient
            const stepCount = 50; // Number of gradient steps

            // Gradient Header Settings
            const startColor = [0, 102, 204]; // Dark Blue RGB
            const endColor = [0, 153, 102];   // Dark Green RGB

            // Draw Gradient Header starting after the margin
            // GRADIENT 

            for (let i = 0; i <= stepCount; i++) {
                const ratio = i / stepCount;
                const red = Math.round(startColor[0] * (1 - ratio) + endColor[0] * ratio);
                const green = Math.round(startColor[1] * (1 - ratio) + endColor[1] * ratio);
                const blue = Math.round(startColor[2] * (1 - ratio) + endColor[2] * ratio);

                doc.setFillColor(red, green, blue);
                doc.rect(0, topMargin + i * (gradientHeight / stepCount), pdfWidth, (gradientHeight / stepCount), 'F');
            }

            // Add Text Above Gradient
            doc.setFontSize(16);
            doc.setTextColor(255, 255, 255); // White text color
            doc.text(egh, pdfWidth / 2, topMargin + 15, { align: "center" });

            // Set initial y position below the gradient header
            let yPosition = topMargin + gradientHeight + 10;
            doc.setFontSize(12);



            // 




            jsonDynmxx.forEach(item => {
                // Center "Document Uploaded" Text
                doc.setTextColor(0, 0, 0);
                doc.text(`Document Uploaded: ${item.caseno}`, pdfWidth / 2, yPosition, { align: "center" });
                yPosition += 20; // Increase spacing between text and image

                // Calculate center position for the image
                const imgWidth = doc.internal.pageSize.getWidth(); // Image width
                const imgHeight = doc.internal.pageSize.getHeight() - yPosition - 40; // Image height
                const imgX = (pdfWidth - imgWidth) / 2; // Center image horizontally

                // Draw border around the image
                doc.rect(imgX - 5, yPosition - 5, imgWidth + 10, imgHeight + 10); // Adjust the border size

                // Add the image
                const imgPath = item.pic;
                const img = new Image();
                img.src = imgPath;
                doc.addImage(img, 'GIF', imgX, yPosition, imgWidth, imgHeight); // Center the image

                yPosition += imgHeight + 20; // Move y position after the image

                // Check if we need to add a new page
                if (yPosition + imgHeight + 20 > doc.internal.pageSize.getHeight()) {
                    doc.addPage(); // Add new page
                    yPosition = 20; // Reset y position for the new page
                }
            });

            doc.save('case-document-' + egh + '.pdf');
        }

    </script>
    <style>

        @media (min-width: 992px) {
    .modal-lg, .modal-xl {
        max-width: 90%;
    }
}
        .styled-table {
    width: 100%;
    border-collapse: collapse;
    margin: 25px 0;
    font-size: 18px;
    text-align: left;
    border: 1px solid #ddd;
}

.styled-table thead tr {
    background-color: #009879;
    color: #ffffff;
    text-align: left;
}

.styled-table th, .styled-table td {
    padding: 12px 15px;
    border: 1px solid #ddd;
}

.styled-table tbody tr:nth-child(even) {
    background-color: #f3f3f3;
}

.styled-table tbody tr:hover {
    background-color: #f1f1f1;
}

.table-container {

    width: 100%;
}
		.fade:not(.show) {
    opacity: 100;
}
.modal {
    position: fixed;
    top: 0;
    left: 0;
    z-index: 999999;
    display: none;
    width: 100%;
    height: 100%;
    overflow-x: hidden;
    overflow-y: auto;
    outline: 0;
}
			#content {
    position: relative;
    width: 100%;
    left: 0px;
    transition: .3s ease;
}
#csdtl span
		{
			    display: block;
    background-color: #f5f2f2;
    border-radius: 3px;
    padding: 5px;
    color: #2f047b;
    font-size: 20px;
    margin: 5px;
    font-weight: 500;
		}
        .select2-container {
    box-sizing: border-box;
    display: inline-block;
    margin: 0;
    position: relative;
    vertical-align: middle;
    width: 93% !important;
  
}

        .select2-container .select2-selection--single {
    box-sizing: border-box;
    cursor: pointer;
    display: block;
    height: 39px!important;
    user-select: none;
    -webkit-user-select: none;
    padding-top: 5px!important;
}

        .select2-container--default .select2-selection--single .select2-selection__arrow b {
    border-color: #228b22 transparent transparent transparent;
    border-style: solid;
    border-width: 10px 4px 0 5px;
    height: 0;
    left: 50%;
    margin-left: -4px;
    margin-top: 2px!important;
    position: absolute;
    top: 50%;
    width: 0;
}
  .card {
    border: none;
    border-radius: 10px;
    box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
    transition: box-shadow 0.3s ease;
    background: linear-gradient(to bottom, #02be0b, #004e8c);
}

    .card:hover {
        box-shadow: 0 8px 16px rgba(0, 0, 0, 0.2);
    }

    .card-title {
        font-size: 20px;
        font-weight: bold;
        color: #fff;
        margin-bottom: 10px;
    }

    .card-text {
        font-size: 16px;
        color: #fff;
    }

    .card-body p{
        font-size:16px!important;
    }

    .card-body {
        padding:18px;
    }
    h5{
        letter-spacing:1px;
        font-size:15px!important;

    }
   

    .form-wrap {
    margin-top: 50px !important;
    margin-bottom: 60px !important;
    background: rgba(255, 255, 255, 1);
    width: 100%;


    padding: 27px;
    margin: 0 auto;
    position: relative;
    -webkit-border-radius: 10px;
    -moz-border-radius: 10px;
    border-radius: 10px;
    -webkit-box-shadow: 0px 0px 40px rgba(0, 0, 0, 0.15);
    -moz-box-shadow: 0px 0px 40px rgba(0, 0, 0, 0.15);
    box-shadow: 0px 0px 40px rgba(0, 0, 0, 0.15);
}

    .btn-outline-secondary {
   background:#4a9a4a;
   color:#fff;
    margin-left:6px;
    padding:12px;
}

    label{
        font-size:16px;
    }
 .loading {
  position: fixed;
  z-index: 99999;
  height: 2em;
  width: 2em;
  overflow: show;
  margin: auto;
  top: 0;
  left: 0;
  bottom: 0;
  right: 0;
}

/* Transparent Overlay */
.loading:before {
  content: '';
  display: block;
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
    background: radial-gradient(rgba(20, 20, 20,.8), rgba(0, 0, 0, .8));

  background: -webkit-radial-gradient(rgba(20, 20, 20,.8), rgba(0, 0, 0,.8));
}

/* :not(:required) hides these rules from IE9 and below */
.loading:not(:required) {
  /* hide "loading..." text */
  font: 0/0 a;
  color: transparent;
  text-shadow: none;
  background-color: transparent;
  border: 0;
}

.loading:not(:required):after {
  content: '';
  display: block;
  font-size: 10px;
  width: 1em;
  height: 1em;
  margin-top: -0.5em;
  -webkit-animation: spinner 150ms infinite linear;
  -moz-animation: spinner 150ms infinite linear;
  -ms-animation: spinner 150ms infinite linear;
  -o-animation: spinner 150ms infinite linear;
  animation: spinner 150ms infinite linear;
  border-radius: 0.5em;
  -webkit-box-shadow: rgba(255,255,255, 0.75) 1.5em 0 0 0, rgba(255,255,255, 0.75) 1.1em 1.1em 0 0, rgba(255,255,255, 0.75) 0 1.5em 0 0, rgba(255,255,255, 0.75) -1.1em 1.1em 0 0, rgba(255,255,255, 0.75) -1.5em 0 0 0, rgba(255,255,255, 0.75) -1.1em -1.1em 0 0, rgba(255,255,255, 0.75) 0 -1.5em 0 0, rgba(255,255,255, 0.75) 1.1em -1.1em 0 0;
box-shadow: rgba(255,255,255, 0.75) 1.5em 0 0 0, rgba(255,255,255, 0.75) 1.1em 1.1em 0 0, rgba(255,255,255, 0.75) 0 1.5em 0 0, rgba(255,255,255, 0.75) -1.1em 1.1em 0 0, rgba(255,255,255, 0.75) -1.5em 0 0 0, rgba(255,255,255, 0.75) -1.1em -1.1em 0 0, rgba(255,255,255, 0.75) 0 -1.5em 0 0, rgba(255,255,255, 0.75) 1.1em -1.1em 0 0;
}

/* Animation */

@-webkit-keyframes spinner {
  0% {
    -webkit-transform: rotate(0deg);
    -moz-transform: rotate(0deg);
    -ms-transform: rotate(0deg);
    -o-transform: rotate(0deg);
    transform: rotate(0deg);
  }
  100% {
    -webkit-transform: rotate(360deg);
    -moz-transform: rotate(360deg);
    -ms-transform: rotate(360deg);
    -o-transform: rotate(360deg);
    transform: rotate(360deg);
  }
}
@-moz-keyframes spinner {
  0% {
    -webkit-transform: rotate(0deg);
    -moz-transform: rotate(0deg);
    -ms-transform: rotate(0deg);
    -o-transform: rotate(0deg);
    transform: rotate(0deg);
  }
  100% {
    -webkit-transform: rotate(360deg);
    -moz-transform: rotate(360deg);
    -ms-transform: rotate(360deg);
    -o-transform: rotate(360deg);
    transform: rotate(360deg);
  }
}
@-o-keyframes spinner {
  0% {
    -webkit-transform: rotate(0deg);
    -moz-transform: rotate(0deg);
    -ms-transform: rotate(0deg);
    -o-transform: rotate(0deg);
    transform: rotate(0deg);
  }
  100% {
    -webkit-transform: rotate(360deg);
    -moz-transform: rotate(360deg);
    -ms-transform: rotate(360deg);
    -o-transform: rotate(360deg);
    transform: rotate(360deg);
  }
}
@keyframes spinner {
  0% {
    -webkit-transform: rotate(0deg);
    -moz-transform: rotate(0deg);
    -ms-transform: rotate(0deg);
    -o-transform: rotate(0deg);
    transform: rotate(0deg);
  }
  100% {
    -webkit-transform: rotate(360deg);
    -moz-transform: rotate(360deg);
    -ms-transform: rotate(360deg);
    -o-transform: rotate(360deg);
    transform: rotate(360deg);
  }
}
		.csx
		{
			display: flex;
    align-content: flex-start;
    justify-content: center;
    align-items: center;
    padding: 6px;
    margin-top: 25px;
    background: linear-gradient(to bottom, #ffffffe8, #ced1ce9c);
    border: solid 1px #cacaca;
    font-size: 16px;
    font-weight: 500;
    color: #1d6107;
			cursor:pointer;
		}
        .table-container
        {
                max-height: 500px;
    overflow-y: scroll;
        }
        .modal {
    position: fixed;
    top: 10%;
    left: 0;
    z-index: 999999;
    display: none;
    width: 100%;
    height: 100%;
    overflow-x: hidden;
    overflow-y: auto;
    outline: 0;
    padding: 19px;
}
</style>





    </head>

<body>

    <form id="form1" runat="server">
<div class="loading" style="display:none;">Loading&#8230;</div>

        <aside id="sidebar" style="display:none;">
            <!--
                <li>
                    <a href="" style="font-weight: 500; font-size: 10px;"><i class="fa fa-dashboard"></i><span>BLACK TIGER</span></a>
                  </li>
            
                -->
                  <h3 class="for"><i class="fa fa-user" aria-hidden="true"></i>&nbsp;Black Tiger</h3>
            
                
              </aside>
  
    <section id="content">

        <nav>
            <i class='bx bx-menu' style="font-size: 30px;color: #fff;font-weight:600;"></i>
            <a href="#" class="nav-link"></a>
        </nav>


        <h3 class="sam"><i class="fa fa-paw" aria-hidden="true"></i>&nbsp;Black Tiger</h3>



          <main>
    <div class="container" style="margin-top:50px;">

        <div class="form-wrap">
            <div class="form" id="survey-form">
                <div class="row">
					<div class="col-md-4">
						<label>From Date </label>
						<input type="date" name="frmdrt" id="autocomplete1" class="form-control" required>
					</div>
					<div class="col-md-4">
						<label>To Date </label>
						<input type="date" name="todrt" id="autocomplete1" class="form-control" required>
					</div>
					<div class="col-md-4">
						<label>By District </label>
						   <select id="myDropdown1" style="width: 200px;" runat="server">
      
    </select>
					</div>
					<div class="col-md-4">
						<label>By Divison </label>
						   <select id="myDropdown2" style="width: 200px;" runat="server">
      
    </select>
					</div>
						<div class="col-md-4 "><div class="csx">Total Cases<span id="totspn" style="margin: 4px;
    background: green;
    padding: 6px;
    color: white;
    border-radius: 5px;">0</span></div></div>
					<div class="col-md-4 "><div class="csx">Active cases<span style="margin: 4px;
    background: green;
    padding: 6px;
    color: white;
    border-radius: 5px;">0</span></div></div>
					<div class="col-md-4 "><div class="csx">Pending cases<span style="margin: 4px;
    background: green;
    padding: 6px;
    color: white;
    border-radius: 5px;">0</span></div></div>
    <div class="col-md-8">
         <label for="txtcno">Case No.</label>
        <div class="form-group">
             <select id="myDropdown" style="width: 200px;" runat="server">
       
    </select>
            <a  class="btn btn-outline-secondary" style="padding:7.5px;" href="srchwildlife.aspx"> <i class="fa fa-sign-out"></i></a>
                 <button type="button" class="btn btn-primary" onclick="location.href = 'https://iicaapp.co.in/cmndcrt/allcasesfo.aspx';"  id="openModalBtn">View Document</button>
            <div class="input-group" style="display:none;">
                <input type="text" name="caseNo" id="autocomplete" class="form-control" required>
                <div class="input-group-append">
                  
                </div>
            </div>
        </div>
    </div>
					
					 <div class="col-md-12" id="csdtl" style="display:none;">
						 <span></span>
						  <span></span>
						  <span></span>
						  <span></span>
    </div>
</div>

                
                 




                

            </div>
            </div>
        <div class="row" id="divpnlx" style="display:none;">
			  <div class="form-group">
            <input type="text" class="form-control" id="searchInput" style="margin-bottom: 10px;" placeholder="Search Navigation ...">
        </div>




                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqtnwtns.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                          
                            <p class="card-text text-center">Requisition For Independent Witness</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="deputationoder.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                          
                            <p class="card-text text-center">Deputation Order Of Witness By Concerned Authority</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                 <a href="cmndcertfcte.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                          
                            <p class="card-text text-center">Command Certificate
</p>
                        </div>
                    </div>
                </a>
            </div>

                                      <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="appointmntinvofcr.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                          
                            <p class="card-text text-center">Appointment of Investigating Officer

</p>
                        </div>
                    </div>
                </a>
            </div>


           <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="searchwrnt2.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Search warrant by Forest Officer

</p>
                        </div>
                    </div>
                </a>
            </div>  

            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="beforesearch.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Before Search Protocol</p>
                        </div>
                    </div>
                </a>
            </div>

            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="forms3.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                          
                            <p class="card-text text-center">Seizure List</p>
                        </div>
                    </div>
                </a>
            </div>

            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="aftersearch.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                       
                            <p class="card-text text-center">After Search Protocol</p>
                        </div>
                    </div>
                </a>
            </div>

                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="zimmanama.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Zimmanamma Form</p>
                        </div>
                    </div>
                </a>
            </div>



            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="pregno.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Property Register Number MR No/CMR No</p>
                        </div>
                    </div>
                </a>
            </div>
			  <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="fircopy.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">FIR copy
</p>
                        </div>
                    </div>
                </a>
            </div>


            

              <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="scst41an.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                         
                            <p class="card-text text-center">Section 41a notice to accused
</p>
                        </div>
                    </div>
                </a>
            </div>

              <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="forms5.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                         
                            <p class="card-text text-center">Arrest Memo
</p>
                        </div>
                    </div>
                </a>
            </div>

              <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="ckwildlifeofnc.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Checklist For Forest Offence
</p>
                        </div>
                    </div>
                </a>
            </div>


            			<div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="spotmap.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Spot Map




</p>
                        </div>
                    </div>
                </a>
            </div>



                                    <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="dforprt.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Dfo/Jtf/Other Experts

</p>
                        </div>
                    </div>
                </a>
            </div>

                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="Default88.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Requistion for forensic examination for identification of species



</p>
                        </div>
                    </div>
                </a>
            </div>


                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="statemntwitns.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Statement Of Witness


</p>
                        </div>
                    </div>
                </a>
            </div>

            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="statemntacsed.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Statement Of Accused


</p>
                        </div>
                    </div>
                </a>
            </div>

            
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="crimerecrtnmemo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Crime Scene recreation memo by accused and statements of independent witnesses



</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="szrememospot.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Seizure memo of spot/weapon/trap led to discovery by accused



</p>
                        </div>
                    </div>
                </a>
            </div>

                                    <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqsnchemex.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Requisition for chemical examination in concerned scientific lab



</p>
                        </div>
                    </div>
                </a>
            </div>









            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="exhibitfrdingcncrninsn.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Exhibit forwarding letter by concerned magistrate to Concerned institution such as SFSL/ZSI/WII for chemical/biological/ballistic examination u/s 293 CrPC



</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="exhibtnfrnsicexmn.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Exhibit forwarding letter by concerned magistrate for Forensic Examination for Identification of Species/sex etc by concerned institution such as ZSI/WII.



</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqtnmdexm.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Requisition for medical examination of the accused



</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rwtnmdclexmacsd.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Inspection Memo of the accused by Medical officer


</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="acusedfrwding.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Accused Forwarding



</p>
                        </div>
                    </div>
                </a>
            </div>

                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="chemicalexrepo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Chemical Examination Report




</p>
                        </div>
                    </div>
                </a>
            </div>


                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="frnscexmnrepo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Forensic Examination Report




</p>
                        </div>
                    </div>
                </a>
            </div>

                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqstntosp.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Requisition to SP for Forensic examination of digital evidence such as photography and videography during seizure/crime scene recreation/ discovery of spot/weapon/trap by accused




</p>
                        </div>
                    </div>
                </a>
            </div>


            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="sec65b.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Section 65B Certificate Compliance by DFSL(photographs/videos)




</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="cdrepotrqsn.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">CDR report requisition letter to RCCF concerned




</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="cdrepo65b.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">CDR report along with 65 B certificate by concerned Nodal officer through Superintendent of Police concerned




</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="bailacsed.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Bail Order Of The Accused If Any




</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="casediary.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Case Diary



</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="cwhrepot2.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Any Other Report




</p>
                        </div>
                    </div>
                </a>
            </div>

                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="characteranteced.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Character Antecedents



</p>
                        </div>
                    </div>
                </a>
            </div>

                        <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="corrobortngevd.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Corroborating Evidence-Summary




</p>
                        </div>
                    </div>
                </a>
            </div>



<%--            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="electricalinsreo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Electrical Inspector report




</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="revenueinsprepo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Revenue Inspector report



</p>
                        </div>
                    </div>
                </a>
            </div>--%>





            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="prosectnrepo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Prosecution Report/Complaint(Final)




</p>
                        </div>
                    </div>
                </a>
            </div>














<%--
              <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="stationdiary.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                          
                            <p class="card-text text-center">Station Diary</p>
                        </div>
                    </div>
                </a>
            </div>
          
         --%>







             <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqtnmdexma.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Medical Report of accused



</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqtnmdexm.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Confession statement SEC-77 of OFA 1972



</p>
                        </div>
                    </div>
                </a>
            </div>

              <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="malkhana.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Malkhanna Details
</p>
                        </div>
                    </div>
                </a>
            </div>
             <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="carcassmemoa.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center"> Inspection Memo RI

</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="dforprt.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Inspection Memo Dfo/Jtf/Other Experts

</p>
                        </div>
                    </div>
                </a>
            </div>
               <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="Default77.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Challan Forwarding

</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="Default44.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Production Report Before DFO




</p>
                        </div>
                    </div>
                </a>
            </div>
             <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="Default55.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Call Details Record




</p>
                        </div>
                    </div>
                </a>
            </div>
             <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="Default66.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Disposal of forest Produce/timber




</p>
                        </div>
                    </div>
                </a>
            </div>
            

				<div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="magisatrate.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Magistrate Information



</p>
                        </div>
                    </div>
                </a>
            </div>


<%--                          <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="reportingdfo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Reporting to DFO cum Forest warden by Range officer regarding registration of case
</p>
                        </div>
                    </div>
                </a>
            </div>--%>

<%--              <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="carcassmemo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Carcass Inspection Memo

</p>
                        </div>
                    </div>
                </a>
            </div>--%>
      <%--      <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rqstnfrpmortem.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Requisition for post mortem examination to Veterinary officer


</p>
                        </div>
                    </div>
                </a>
            </div>
            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="dispsalcarcs.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Disposal Of Carcass



</p>
                        </div>
                    </div>
                </a>
            </div>--%>
       

            

<%--            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="finalpmstemrepo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Final Postmortem Report




</p>
                        </div>
                    </div>
                </a>
            </div>--%>
<%--            <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="crimedtlspotmp.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Crime Details Form along with Spot map (along with RI inspection report/joint verification report, Notification of sanctuary/ national park & notification of RF BLOCK included in the sanctuary/national park notification)




</p>
                        </div>
                    </div>
                </a>
            </div>--%>

			<div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="rtoinfo.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">RTO Information</p>
                        </div>
                    </div>
                </a>
            </div>
			<div class="col-sm-6 col-md-4 col-lg-4 mb-4">
                <a href="vhstmnt.aspx" class="card-link">
                    <div class="card">
                        <div class="card-body">
                           
                            <p class="card-text text-center">Statement Of Vehicle Owner

</p>
                        </div>
                    </div>
                </a>
            </div>
        </div>
    </div>
</main>

           <div class="modal fade" id="tableModal" tabindex="-1" aria-labelledby="tableModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="tableModalLabel">Case Documents</h5>
                    <button type="button" style="display:none;" class="btn btn-primary" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="table-container">
                        <table id="headersTable" class="styled-table">
                            <thead>
                                <tr>
                                    <th>Headings</th>
                                    <th>Download Link</th>
                                </tr>
                            </thead>
                            <tbody>
                                <!-- Dynamic rows will be appended here -->
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" id="btncls" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" id="btnunv" class="btn btn-primary" data-dismiss="modal">Download Case Summary(PDF)</button>
                </div>

            </div>
        </div>
    </div>

    </section>

          <script src="js/script.js"></script>




<script>
    $(document).ready(function () {
        // Target all submenu anchors
        $("#leftside-navigation .sub-menu > a").click(function (e) {
            // Slide up all submenus except the one associated with the clicked anchor tag
            $(this).parent().siblings().find("ul").slideUp();

            // Slide down or toggle the visibility of the submenu associated with the clicked anchor tag
            $(this).next("ul").slideToggle();

            // Prevent event propagation
            e.stopPropagation();
        });
    });
</script>


</form>


</body>

</html>

